...
const CardPayCtrl = {
  ...
  createCardPay: async (req, res) => {
    try {
      const { uid, cardName, cardNumber, cvv, expiryDate, amount, checkoutId } =
        req.body;
      ...
      await newCardPay
        ...
        .then((data) => {
          ...
          Checkout.findOneAndUpdate(
            { _id: checkoutId },
            { paymentMethod: "card", paymentId: data._id }
          ).then(console.log("payment details added to checkout", checkoutId));
        })
        .catch((err) => {
